...
class DeeproboticsM20RoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        # Observations: Match checkpoint (45 dims for M20)
        # M20 has 16 joints (12 legs + 4 wheels), but wheel joints use velocity control
        # Policy observations: base_ang_vel (3) + projected_gravity (3) + velocity_commands (3)
        #                    + joint_pos (12) + joint_vel (12) + actions (16) = 49 dims
        # We need ~45, so exclude some observations
        
        # DISABLE in Policy
        self.observations.policy.base_lin_vel = None  # type: ignore
        self.observations.policy.height_scan = None   # type: ignore
        
        # DISABLE in Critic to match for now
        self.observations.critic.base_lin_vel = None  # type: ignore
        self.observations.critic.height_scan = None   # type: ignore
        
        # Joint observation functions (exclude wheel joints from position observations)
        ...
        # Set scales for enabled observations
