...
class DeeproboticsLite3RoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        # Observations: Match checkpoint EXACTLY (45 dims for actor, 393 for critic)
        # Actor needs: base_ang_vel (3) + projected_gravity (3) + velocity_commands (3) 
        #            + joint_pos (12) + joint_vel (12) + actions (12) = 45 dims
        
        # DISABLE extras in Policy
        self.observations.policy.base_lin_vel = None  # type: ignore
        self.observations.policy.height_scan = None   # type: ignore
        
        # DISABLE ALL in Critic to match actor for now (both will be 45)
        # Later we can enable more for critic if needed
