...
def camera_follow(env):
    ...
    # Clamp camera height to prevent going underground
    camera_pos[2] = torch.clamp(camera_pos[2], min=0.5)
    window_size = 50
    ...
    # Also clamp the smoothed camera position
    smooth_camera_pos[2] = torch.clamp(smooth_camera_pos[2], min=0.5)
